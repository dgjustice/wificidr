...
def answer(n, b):
    ...
    n_minion = n
    c = 0
    while n_minion not in minions and c < 50:
        minions.append(n_minion)
        a_minion = next_minion(n_minion, b)
        if a_minion == n_minion:
            return 1
        n_minion = a_minion
        c += 1
    return len(minions) - minions.index(n_minion)

def test_answer():
    assert answer("1211", 10) == 1
    assert answer("210022", 3) == 3
