...
def test_insert(test_heap):
    """Test heap insert and min."""
    assert test_heap.heap[0] == min(test_heap.heap)

@pytest.mark.parametrize("count", range(MAX_ITER // 2))
def test_pop_is_min(count, test_heap):
    """Test pop is minimum value."""
    for _ in range(len(test_heap.heap)):
        min_val = min(test_heap.heap)
        popped_val = test_heap.pop()
        assert min_val == popped_val
...
def test_pop_all_sorted(test_heap):
    ...
    assert test_list == sorted(set(test_list))
